...
def datos_consulta(db: Session, ficha: Ficha) -> dict:
    ...
    return {
        ...
        "repuestos": repuestos,
        "importes": {
            "total_repuestos": f"{ficha.total_repuestos:.2f}",
            "costo_servicio": f"{ficha.costo_servicio:.2f}",
            "total": f"{ficha.total:.2f}",
            "adelanto": f"{ficha.adelanto:.2f}",
            "saldo": f"{ficha.saldo:.2f}",
